...
- **El informe al MPC.** Mediste tu asteroide en Astrometrica y ahora toca
  montar a mano un fichero de 80 columnas, cruzando los dedos para que ningún
  campo se haya desplazado.
- **El post que nunca existió.** Capturaste algo grande el mes pasado. Nunca
  lo contaste — y ahora mismo no sabrías decir qué noche fue, cuántas tomas ni
  con qué filtro.
...
3. **Contar tu ciencia** — posts bilingües (ES/EN), un tuit y gráficos listos
   para compartir, redactados con los *datos reales de tu sesión*.
...
| **Opcional** | **CCDciel** en ejecución local, si quieres que la app maneje montura/cámara · cuatro claves API gratuitas, cada una desbloquea un extra (reporte NEOfixer, resolución ciega Astrometry.net, bot de TNS, token AAVSO) — ver *Integraciones opcionales* |
...
4. **Haz clic en cualquier objetivo** — verás su ficha completa explicada. ¿Te
   gusta lo que ves? Un botón: **Crear proyecto**.
...
Cómo se siente usar NightScribe, en seis escenas:
...
3. **Planificar y capturar.** Dimensionas la sesión: 40 tomas × 45 s, filtro
   ...
   app.
4. **Procesado.** Mides el apilado en tu herramienta de astrometría habitual y
   **pegas** las líneas en el proyecto. NightScribe las valida (formato, tu
   código MPC, designación) y empaqueta el **informe para el MPC**, listo
   para adjuntar al correo que envías tú.
5. **Publicar.** El post bilingüe y el tuit ya están redactados con los datos
   reales de la sesión — fecha, 40×45 s, filtro — más los gráficos de órbita y
   cielo. Copias, pegas, listo.
6. **Las semanas siguientes.** El diario recuerda esa noche por sí solo. La
   supernova que confirmaste mantiene su propia curva de luz, y el panel te
   avisa cuando han pasado tres noches sin revisitarla. Una tarde aparece un
   chip en Esta noche: *«R CrB está cayendo — tu vigilia»*.
...
- **NEOs** — asteroides cercanos a la Tierra de la lista *específica de tu
  sitio* de NEOfixer: score, prioridad, coste en minutos de telescopio,
  velocidad aparente, incertidumbre en el cielo, indicadores
  NEOCP/riesgo-de-impacto/radar/NHATS y —para los NEOCP aún sin confirmar—
  **elementos orbitales preliminares con sus sigmas** (incertidumbre por
  elemento), calculados desde la astrometría del MPC.
- **Cometas** — magnitudes observadas en vivo de COBS, fechas de perihelio,
  indicadores de actividad.
- **PCCP** — la página de confirmación de posibles cometas del MPC: objetos
  reportados como asteroides que podrían ser cometas, con su comet-score.
  Llegar primero importa.
- **Supernovas** — la lista de descubrimientos recientes de Rochester
  Astronomy.
- **Tránsitos de exoplanetas** — los tránsitos de esta noche según ExoClock
  ...
  prioridad y deriva O-C (cuánto se desvía la hora observada del tránsito
  respecto a la predicción — la razón científica de reobservar).
- **Alertas de aproximación** — próximos acercamientos de ESA NEOCC: distancia
  de paso, tamaño estimado, magnitud máxima.
- **Estrellas HADS** — variables δ Scuti de gran amplitud: estrellas que
  pulsan tan rápido (periodos de 1–5 h) y tan fuerte (≥ 0,3 mag) que **las
  ves variar en una sola sesión**. Del catálogo vivo que mantiene P. Wils.
- **Variables y deberes** — miembros de tus campañas que tocan por cadencia,
  estrellas con un evento en curso, estrellas cerca de un extremo predicho
  por el VSX, tus vigilias permanentes y el canal editorial AAVSO (alertas
  del foro y campañas activas) — siempre que la estrella esté arriba esta
  noche.
...
- **Magnitud límite** — política híbrida: corte duro para supernovas, cometas
  y tránsitos; aviso suave `⚠ mag>N` para NEOs/PCCPs (sus predicciones de
...
- Con CCDciel conectado: **Apuntar telescopio** mueve a la posición *recién
  ...
  incertidumbres.
- Procesa en tu herramienta habitual, **pega la astrometría** en el proyecto y
  obtén un **informe MPC** validado y empaquetado (80 columnas o ADES PSV,
  con tu código MPC y la designación comprobados). El correo lo envías tú; el
  fichero ya está listo.
- La **animación de movimiento** es la prueba de fuego para divulgar: tus
  tomas alineadas por las estrellas, un marcador cabalgando la posición
  *predicha* con la tasa/PA previstas y la fuente de la efeméride en el pie.
  Si un punto permanece bajo el marcador mientras las estrellas derivan — ese
  es tu objeto.
...
- **Seguimiento multi-noche**: un proyecto de supernova vive semanas o meses.
  ...
  (AstroImageJ/Tycho/CSV) o importación de fichero. La **curva de luz** dibuja
  tus puntos frente a las plantillas típicas de cada tipo (Ia, II-P/L, Ib/c,
  SLSN, kilonova), la app calcula un análisis de campaña indicativo con
...
- **Campañas**: una campaña es el compromiso compartido de un grupo —
  objetivo científico, protocolo (cadencia, filtros, estrellas de
  comparación), URLs de reporte/datos. Una campaña, muchos proyectos asociados
  (borrar la campaña los libera, nunca los borra). Un miembro *toca* cuando su
  última sesión tiene `cadence_nights` noches y aterriza en Esta noche
...
- **El Sol ahora** — los últimos canales de NASA SDO (corona a 193/304/171 Å,
  manchas, magnetograma), el mapa de regiones activas de NOAA, los índices
  (número de manchas, flujo de radio F10.7, Kp, la fulguración más fuerte de
  la semana), un panel PNG con marca de agua listo para tus redes y un
  borrador bilingüe del post «el cielo hoy». Una línea de «impacto en tu
  noche» lo conecta con la observación: una Luna brillante perjudica a los
  objetos débiles, un Kp alto puede significar auroras.
...
astrométrico** (slew + captura + resolución de placas + corrección) y
**captura en vivo** (prepara el plan guardado del proyecto, arranca la
secuencia).
...
y datos físicos cruzados desde JPL SBDB/Horizons/CAD, SIMBAD, TNS y el NASA
...
El post no es un añadido; es el último paso de cada proyecto:

- Borradores bilingües (ES/EN) más un tuit, redactados con los **datos reales
  de la sesión** — fecha, N×t, filtro — no una plantilla genérica.
- El inventario de gráficos: vista de la órbita, cielo nocturno con tu
  silueta de horizonte, campo del survey, curva de luz frente a plantillas,
  línea de tiempo del tránsito, panel del Sol, antes/después y blink GIF/MP4
  de la supernova, rastro de movimiento del NEO, animación de evolución de la
  SN. Listos para adjuntar.
...
**Datos de objeto (alimentan las fichas y los posts):**
...
|---|---|
| **CCDciel** (local, host/puerto) | La pestaña Observatorio: estado, apuntar telescopio, goto astrométrico, captura en vivo. Solo con CCDciel en ejecución |
...
- **No reduce tus tomas.** La calibración, el apilado y la medida
  astrométrica/fotométrica ocurren en tus herramientas habituales
  (Astrometrica, AstroImageJ, EXOTIC…). NightScribe prepara la noche, valida y
  empaqueta los resultados, y escribe la historia.
- **El informe MPC se empaqueta y se valida; el correo lo envías tú.**
- **Formatos de exportación**: el formato de secuencias CCDciel está validado
  contra una exportación real de CCDciel, y las efemérides TheSkyX/Cartes du
