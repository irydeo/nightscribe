...
- **Proyecto**: entidad persistente por objetivo, con tipo (`sn`, `neo`, `comet`,
  `pccp`, `transit`), estado (`active`/`done`/`archived`) y **pasos guiados por tipo**:
  Plan → Captura → Procesado → Análisis → Publicar. El contexto (coords, mag, rate,
...
**Persistencia** (migración `user_version` 0→1 en `core/db.py`, per ADR-002):
...
a la nueva navegación. Cometas y tránsitos reutilizan el esqueleto de 5 pasos más
...
- **Project**: persistent entity per target, with kind (`sn`, `neo`, `comet`, `pccp`,
  `transit`), status (`active`/`done`/`archived`) and **guided steps per kind**:
  Plan → Capture → Process → Analyse → Publish. Context (coords, mag, rate, window,
...
**Persistence** (`user_version` 0→1 migration in `core/db.py`, per ADR-002):
...
Comets and transits reuse the 5-step skeleton later. Work runs in phases (see §7 of
