# Fotometría de precisión en NightScribe: qué hace falta
...
quiere entender los conceptos y decidir hasta dónde quiere llegar
(primera parte, sin matemáticas); y a una IA o desarrollador que un día
implemente las piezas que faltan (apéndice técnico, al final).

Documentación de lo que ya existe y cómo funciona:
[PHOTOMETRY.es.md](PHOTOMETRY.es.md). Este documento parte de ahí y mira
hacia delante.
...
De lo gratis a lo heroico; cada peldaño «compra» una mejora típica:

| Peldaño | Esfuerzo | Qué compra |
|---|---|---|
| Placa bien reducida (bias, darks, **flats**) | observación | un campo plano: sin esto, 1–5 % de error según la posición |
| Comparaciones de color parecido al objetivo | un clic bien elegido | quita la mayor parte del término de color |
| Apertura que sigue al seeing de la noche | poco código | SNR óptima y consistencia noche a noche |
| Cielo bien estimado (mediana robusta o plano) | poco código | la SN deja de medirse «con galaxia incluida» |
| Nivel de saturación real de tu cámara | una línea de configuración | ninguna estrella cortada se cuela como buena |
| Término de color ajustado con las comps | matemática | la respuesta de tu equipo deja de sesgar el cero |
| Sustracción de la galaxia huésped (SNe) | código moderado | en núcleos galácticos: de 0,05–0,15 a 0,03–0,05 mag |
| Normalización por frame + detrending (series) | código moderado | el requisito de los tránsitos: 0,001–0,005 mag relativo |

### 3. Tres escenarios, cifras honestas

* **Variable en buena noche, placa reducida**: hoy el flujo de series da
  Δmag con 0,02–0,05 mag de precisión total; con la apertura adaptativa,
  el término de color y la check vigilando, **0,01–0,02 mag** es
  realista. A partir de ahí manda la calibración del color, y bajar más
  exige medir los coeficientes de transformación propios de tu cámara
  sobre campos estándar (otra liga, anotada como v2).
* **Supernova pegada a un núcleo galáctico**: el enemigo no es el ruido,
  es la galaxia. Con cielo por plano en el anillo mejora algo; con
  **sustracción de la huésped** (la referencia PS1 ya alineada por el
  blink se resta, escalada para que las estrellas desaparezcan y solo
  quede la SN) se pasa de 0,05–0,15 a **0,03–0,05 mag**.
...
* **La estrella check es el semáforo**: se mide como si fuera el
  objetivo y se compara con su valor de catálogo. Si ella se mueve, la
  culpa no es de la variable: la noche, la placa o la secuencia no son
  de fiar. Si solo se mueve el objetivo, eso es astrofísica.
* **Saturación con nivel real**: «casi saturada» no existe. El techo
  debe venir de la tarjeta SATURATE o de la configuración de tu cámara,
  no estimado del propio frame.
* **El error reportado vs. el total**: el programa debe decirte si su
  error incluye solo el ruido o también la dispersión de las comps y el
  ajuste de color. Fiarse del primer número como si fuera el segundo es
  el error más común.
...
| Medida calibrada en una placa (punto cero con las comps) | Plan firmado: `PLANS/ufe-photometry.md` (fase G) |
| Término de color, cielo en gradiente, apertura por FWHM, saturación real, error total, semáforo check | Falta: piezas A–G del apéndice |
| Sustracción de galaxia huésped | Falta: pieza B2 (el blink ya alinea la referencia) |
| Serie normalizada por frame + detrending para tránsitos | Falta: piezas T1–T8; decisión ADR-015 por reabrir o acotar |

---

## Apéndice: para la IA implementadora (o el desarrollador)
...
offscreen con `np.random.default_rng(<semilla>)` fija (el flake de
`test_series.py` no se repite); los diálogos legacy nunca se tocan; docs
en lenguaje natural (la regla de estilo de siempre: dos puntos, comas y
punto y coma; la semirraya solo para rangos numéricos). Punto de
partida: `docs/PLANS/ufe-photometry.md` (fase G: medida calibrada en
placa; este apéndice es su extensión de calidad).
